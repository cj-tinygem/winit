    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            let new_outer_rect: RECT;
            {
                ...
                // adb-on patch: while the window straddles two monitors during a drag,
                // MonitorFromWindow still reports the old monitor and the nudge below pushes the
                // window back, producing a DPI ping-pong (winit#3040, slint#11073). The monitor that
                // owns the suggested rect is the one the new DPI belongs to.
